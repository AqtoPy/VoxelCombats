...
@onready var player_info_menu = $PlayerInfoMenu

@onready var player_name_edit = $PlayerInfoMenu/NameEdit
@onready var clan_tag_edit = $PlayerInfoMenu/ClanTagEdit
@onready var balance_label = $PlayerInfoMenu/BalanceLabel
@onready var vip_button = $PlayerInfoMenu/VIPButton
@onready var vip_status_label = $PlayerInfoMenu/VIPStatusLabel
...
@onready var vip_price_label = $PlayerInfoMenu/VIPPrice
@onready var purchase_button = $PlayerInfoMenu/VIPButton
...
@onready var promo_code_edit = $PlayerInfoMenu/Promo
...
@onready var level_label = $PlayerInfoMenu/LevelLabel
@onready var xp_bar = $PlayerInfoMenu/XPBar
...
@onready var model_rotation_speed = 10.0

func _ready():
    _setup_directories()
    _load_player_data()
    _check_vip_expiration()
    _generate_player_id()
    _setup_ui()
    _connect_signals()
    load_servers()
...
func _process(delta):
    ...
    if model_viewport.visible:
        model_viewport.get_node("Model").rotate_y(delta * model_rotation_speed)

func _setup_model_view():
    var model_scene = preload("res://models/player_model.tscn").instantiate()
    model_viewport.add_child(model_scene)
...
func _check_vip_expiration():
    if player_data["is_vip"] and player_data["last_played_date"] != "":
        var last_date = Time.get_datetime_dict_from_datetime_string(player_data["last_played_date"])
        var current_date = Time.get_datetime_dict_from_system()
        
        # Calculate days passed
        var days_passed = (OS.get_unix_time() - Time.get_unix_time_from_datetime_dict(last_date)) / 86400
        
        if days_passed > 0:
            player_data["vip_days"] = max(0, player_data["vip_days"] - days_passed)
    ...
    player_data["last_played_date"] = Time.get_datetime_string_from_system()
    _save_player_data()
#endregion
...
func _setup_ui():
    ...
    clan_tag_edit.text = player_data["clan_tag"]
    _update_balance_ui()
    ...
    case_price_label.text = "Цена: 500$"
...
func _connect_signals():
    ...
    $PlayerInfoMenu/BackButton.pressed.connect(func(): _switch_menu("main"))
...
func _switch_menu(menu_name: String):
    ...
    player_info_menu.visible = (menu_name == "player_info")
#endregion
...
func _apply_case_reward(reward: Dictionary):
    match reward["type"]:
        ...
        "character":
            character_shop_data[reward["character"]]["unlocked"] = true
...
func _start_game():
    ...
    if is_server:
        game_scene.init_host()
    else:
        game_scene.init_client()
#endregion
